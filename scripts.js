...
document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch("tabela.json", { cache: "no-store" })
    .then(response => {
      if (!response.ok) throw new Error("Falha ao carregar tabela.json: " + response.status);
    ...
    .catch(err => {
      console.error(err);
      tbody.innerHTML = `<tr><td colspan="5">Erro ao carregar dados. Veja o console.</td></tr>`;
    });
});
